...
export default function ProjectDetails() {
  ...
  const handleCloseChangeModal = () => {
    (
      document.getElementById("change-task-modal") as HTMLDialogElement
    )?.close();
  };

  const handleCloseDeleteModal = () => {
    (
      document.getElementById("delete-task-modal") as HTMLDialogElement
    )?.close();
  };
  ...
  const fetchAssignments = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `api/assignment-users/${selectedTask?.id}`,
        {
  ...
  const addAssignment = async (userId: string) => {
    ...
    try {
      await axios.post(
        `api/assignment-users/${selectedTask?.id}/${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <>
          <div className="card card-compact flex flex-row font-semibold text-xl w-full h-fit px-3 pt-2 pb-3 bg-base-100 bg-opacity-40 shadow-xl mb-4 rounded-lg">
            ...
            <div className="absolute right-2 top-2 space-x-1">
              <button
                className="btn btn-sm bg-gradient-to-r text-white from-green-500 via-green-600 to-green-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80"
                onClick={() =>
                  (
          ...
          <div className="flex flex-row">
            <div className="w-full h-screen-minus-10.5rem overflow-y-auto">
              {sortedTasks.map((task) => (
                <div
                  key={task.id}
                  className="card card-compact flex flex-row items-center space-x-2 font-semibold text-xl w-full h-fit px-3 pt-2 pb-3  hover:bg-base-100 hover:bg-opacity-40 hover:shadow-xl rounded-lg relative group"
                  onClick={() => {
                ...
                >
                  ...
                  <div className="absolute right-2 top-2 space-x-1">
                    <button
                      className="btn btn-circle btn-sm btn-ghost opacity-0 transition-opacity duration-150 group-hover:visible group-hover:opacity-100"
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedTask(task);
                        (
                          document.getElementById(
                            "change-assignments-modal",
                          ) as HTMLDialogElement
                        )?.showModal();
                      }}
                    >
                ...
                {assignments.length > 0 && (
                  <>
                    ...
                    {assignments.map((user) => (
                      <div
                        key={user.id}
                        className="flex flex-row items-center space-x-2 w-full h-fit space-y-1 relative group"
                      >
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="w-fit bg-gray-100 text-gray-800 text-xs font-medium px-1.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                            {user.username}
                          </span>
                        </div>
                      </div>
              ...
              <form className="flex-grow">
                ...
                <div className="relative">
                  ...
                  <input
                    type="search"
                    id="default-search"
                    className="block w-full p-3 ps-9 mt-2 text-sm text-gray-100 shadow-xl rounded-lg focus:ring-green-500 focus:border-green-500  bg-gray-700 bg-opacity-50"
              ...
              <div className="h-[22rem] overflow-y-auto">
                {filteredMembers.map((user) =>
                  assignments.find((m) => m.username === user.username) ? (
                    <div
                      key={user.id}
                      className="card card-compact flex flex-row items-center space-x-2 w-full h-fit px-3 pt-2 pb-3 space-y-1 bg-base-100 bg-opacity-40 hover:shadow-xl hover:bg-gray-800 hover:bg-opacity-30 rounded-lg relative group"
                    >
                      <div className="w-8 h-8 flex rounded-full row justify-center items-center border border-gray-400">
                        <i className="fi fi-sr-user flex"></i>
                      </div>
                      ...
                      <div className="absolute right-2 top-4 space-x-1">
                        <button
                          className="btn btn-circle btn-sm btn-ghost btn-active"
                          disabled={loading}
                          onClick={() => {
