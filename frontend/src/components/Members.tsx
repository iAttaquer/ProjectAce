...
export default function Members() {
  ...
  const addMember = async (userId: string) => {
    ...
    try {
      await axios.post(
        `api/project-teams/${project?.id}/${userId}`,
        {},
        {
          headers: {
    ...
    <div className="w-1/3 flex flex-col pl-3">
      <div className="flex flex-row space-x-2">
        ...
        <button
          ...
          onClick={() =>
            (
              document.getElementById("add-member-modal") as HTMLDialogElement
            )?.showModal()
          }
        >
        ...
        {members &&
          filteredMembers.map((member) =>
            ...
            member.username === user?.username ? (
              <div
                key={member.id}
                className="card card-compact flex flex-row items-center space-x-2 w-full h-fit px-3 pt-2 pb-3 space-y-1 bg-base-100 bg-opacity-40 shadow-xl rounded-lg relative group"
              >
